...
import { PostReaction, RemlorePost } from '@remlore/types';
import Link from 'next/link';
import { useRef } from 'react';

type PartialPostReaction = Pick<PostReaction, 'type'>;

interface PostProps {
  post: RemlorePost;
  reactionCount: number;
  currentReaction?: PartialPostReaction;
  commentCount: number;
}

export function Post({ post, reactionCount, currentReaction, commentCount }: PostProps) {
  ...
  return (
    <div className="rounded-md bg-card shadow">
      <div className="flex justify-between px-6 py-4">
        <PostVoteClient
          postId={post.id}
          initialReaction={currentReaction?.type}
      ...
      <div className="z-20 bg-secondary px-4 py-4 text-sm sm:px-6">
        <Link href={`/post/${post.id}`} className="flex w-fit items-center gap-2">
